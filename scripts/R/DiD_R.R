...
output_dir <- "output/DiD_R_Summaries/"
...
writeLines(as.character(enhanced_latex_table), "output/tables/Summary_Statistics_Log.tex")
...
kableExtra::save_kable(enhanced_latex_table, file = "output/tables/Summary_Model_Statistics.pdf")
...
rel_months_df <- did_data$rel_month
...
est_did_log_contributors <- feols(log_contributors ~ treated * as.factor(rel_month) | org_name + time_period, data = did_data)

est_did_log_release_count <- feols(log_release_count ~ treated * as.factor(rel_month) | org_name + time_period, data = did_data)
est_did_log_running_total_open_issues <- feols(log_running_total_open_issues ~ treated * as.factor(rel_month) | org_name + time_period, data = did_data)
est_did_log_avg_commits_per_contributor <- feols(log_avg_commits_per_contributor ~ treated * as.factor(rel_month) | org_name + time_period, data = did_data)
est_did_log_average_closure_days <- feols(log_average_closure_days ~ treated * as.factor(rel_month) | org_name + time_period, data = did_data)


summary(est_did_log_average_closure_days)

# EXAMPLE
# data(base_did)
# We interact the variable 'period' with the variable 'treat'
# est_did = feols(y ~ x1 + i(period, treat, ) | id+period, base_inter)
...
est_did_log_contributors_v2 <- feols(log_contributors ~ treated * as.factor(rel_month) + age + active_repositories | org_name + time_period, data = did_data)

summary(est_did_log_contributors)
summary(est_did_log_contributors_v2)

coefplot(est_did_log_contributors, 
         main = "Treatment Effect on  __depvar__",
         xlab = "Relative Month to Treatment",
         cex = 0.7, # Adjusts the size of points and text
         lab.cex = 0.7, # Adjusts the size of axis labels
         lab.min.cex = 0.7, # Adjusts the minimum size of text
         grid = TRUE,
         zero = TRUE)


coefplot(est_did_log_release_count, 
         main = "Treatment Effect on  __depvar__",
         xlab = "Relative Month to Treatment",
         cex = 0.7, # Adjusts the size of points and text
         lab.cex = 0.7, # Adjusts the size of axis labels
...
coefplot(est_did_log_running_total_open_issues, 
         main = "Treatment Effect on  __depvar__",
         xlab = "Relative Month to Treatment",
         cex = 0.7, # Adjusts the size of points and text
         lab.cex = 0.7, # Adjusts the size of axis labels
...
coefplot(est_did_log_avg_commits_per_contributor, 
         main = "Treatment Effect on  __depvar__",
         xlab = "Relative Month to Treatment",
         cex = 0.7, # Adjusts the size of points and text
         lab.cex = 0.7, # Adjusts the size of axis labels
...
coefplot(est_did_log_average_closure_days, 
         main = "Treatment Effect on  __depvar__",
         xlab = "Relative Month to Treatment",
         cex = 0.7, # Adjusts the size of points and text
         lab.cex = 0.7, # Adjusts the size of axis labels
...
# the parallel trends assumption doesn't hold true?


# Event Study Difference-in-Differences (DiD) Model with two-way fixed effects

# Now we can plot the result of the interaction with coefplot
coefplot(est_did,
         main = "Treatment Effect on __depvar__", xlab = "Relative Month to Treatment")
